...
module.exports = {
  ...
  run: async(client, interaction) => {
	...
	const destek = new Discord.ButtonBuilder().setLabel('Destek Sunucum').setStyle('Link').setEmoji('1044325557615202364').setURL('https://discord.gg/chQKbM6ucx');
    const row = new Discord.ActionRowBuilder().addComponents(dvt).addComponents(destek)
    const embed = new EmbedBuilder()
    .setAuthor({ name: "Merhaba, Ben GalacticRP!", iconURL: interaction.client.user.displayAvatarURL({ dynamic: true })})
.setTitle("GalacticRP'i Davet Et!")
.setDescription(`<:bot:1039607042291269703> | GalacticRP'i şimdi sunucuna davet et ve botun tadını çıkar!`)
.setColor('#2F3136')
.setTimestamp()
